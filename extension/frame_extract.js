...
function looksLikeTypingPlaceholder(text) {
  ...
  return (
    /^(thinking|typing|loading|generating|please wait|one moment|hold on|let me|working)\.{0,3}$/i.test(
      tl
    ) ||
    /^\.{2,4}$/.test(tl) ||
    tl === "..." ||
...
function looksLikeIntermediateStatus(text) {
  ...
  if (
    /^(let me|i('ll| will)|allow me|one moment|just a (moment|second|sec)|hang on|hold on|working on|looking into)/i.test(
      tl
    ) &&
    tl.length < 80
  )
...
function classifyBubble(el) {
  ...
  for (let n = el, hops = 0; n instanceof Element && hops < 4; n = n.parentElement, hops++) {
    ...
    if (hasChatCtx) {
      if (
        tokens.some((t) => /^(user|customer|visitor|human|self|me|sender|outgoing|sent)$/.test(t))
      )
        return "user";
      if (
        tokens.some((t) =>
          /^(assistant|bot|agent|ai|system|incoming|received|operator|support)$/.test(t)
        )
      )
        return "bot";
    }
  ...
  try {
    ...
    if (flexDir === "row-reverse" || parentFlexDir === "row-reverse") return "user";
...
function extractFromRoleLog() {
  ...
  return fallback && !looksLikeFooter(fallback) && !looksLikeTypingPlaceholder(fallback)
    ? fallback
    : null;
}
...
function extractFromAssistantBubbles() {
  ...
  for (const candidate of sorted) {
    ...
    const t = textOf(inner);
    if (t.length && !looksLikeFooter(t) && !matchesUserMessage(t) && !looksLikeTypingPlaceholder(t))
      return t;
  }
  return null;
}
...
function extractFromAriaLiveRegion() {
  ...
  for (const region of regions) {
    ...
    if (
      ...
      t.length < 50_000 &&
      !looksLikeFooter(t) &&
      !matchesUserMessage(t) &&
      !looksLikeTypingPlaceholder(t)
    )
      return t;
    ...
    if (lastP) {
      ...
      if (
        tp.length >= 12 &&
        !looksLikeFooter(tp) &&
        !matchesUserMessage(tp) &&
        !looksLikeTypingPlaceholder(tp)
      )
        return tp;
    }
  }
...
function extractFromRoleFeed() {
  ...
  if (!last) return null;
  const t = textOf(last);
  if (t.length < 10 || looksLikeFooter(t) || matchesUserMessage(t) || looksLikeTypingPlaceholder(t))
    return null;
  return t;
}
...
function countVisibleMessages() {
  ...
  const msgSels =
    "li, article, [class*='message' i], [class*='bubble' i], [class*='msg-bubble' i], [role='listitem'], [role='article']";
