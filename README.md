...
1. **New evaluators** — add a `.yaml` file (or folder with `evaluator.yaml` + `patterns/`) under `evaluators/agent/` or `evaluators/mcp/`, run `npm run build:catalog`, and commit the updated `catalog.json` files. The engine reads repo root. No TypeScript needed.
