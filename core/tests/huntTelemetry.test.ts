...
test("fetchThreadTrace: a cached trace id returns without hitting the backend", async () => {
  ...
  // The cache is keyed by trace id + the turn's response anchor (a per-attack trace grows across
  // turns), so seed the key the same way fetchThreadTrace will look it up for turn 1 (response "r").
  const cache: TraceCache = new Map([[traceCacheKey("cached-id", "r"), '{"span":"ok"}']]);
