...
description: "No-code red-teaming, entirely client-side in your browser."
---
...
> Currently covers **agent / chatbot red-teaming**. For MCP server red-teaming, use the [CLI](/cli) or [MCP server tool](/mcp).
...
Same evaluator catalog as the CLI — see [evaluators reference](/evaluators).
...
3. Pick a suite (e.g. `owasp-llm-top10`) or specific evaluators — same IDs as the CLI; see [evaluators reference](/evaluators).
4. Click **Start**.
...
Same agent-redteam catalog as the CLI (six suites, ~50 evaluators). Pick by suite or by individual evaluator ID. Full reference: [/evaluators](/evaluators).
...
- **Chat-widget targets only.** The extension needs a detectable chat input + response area in the DOM. Standalone API endpoints and non-chat agents → use the [CLI](/cli).
- **No MCP / live tool-call evaluators.** The judge sees the transcript, not real tool side-effects. For MCP server red-teaming use the CLI's `mode: "mcp"` or the [MCP server tool](/mcp).
